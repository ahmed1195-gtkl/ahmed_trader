      ...
      <div className="container mx-auto px-4 max-w-7xl">
        <div className={`relative flex items-center justify-between bg-zinc-900/40 backdrop-blur-2xl border border-white/10 rounded-2xl px-4 md:px-6 py-3 transition-all duration-500 ${isScrolled ? 'shadow-2xl shadow-yellow-500/10 border-white/20' : ''}`}>
          ...
          <nav className="hidden lg:flex items-center gap-6">
            ...
            {user && isAdmin && (
              <div className="flex items-center gap-2">
                ...
                <Link 
                  to="/admin/challenges" 
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-yellow-500 hover:text-black ${location.pathname === '/admin/challenges' ? 'bg-yellow-500 text-black' : 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20'}`}
                >
          ...
          <div className="flex items-center gap-2 md:gap-4">
            {/* تم إزالة زر تبديل البوت بناءً على طلب المستخدم */}

            {/* تم إزالة زر الإشعارات بناءً على طلب المستخدم */}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {user && isAdmin && (
                    <Link 
                      to="/admin" 
                      onClick={() => setIsSidebarOpen(false)}
                      className={`flex items-center gap-4 p-4 rounded-2xl transition-all ${location.pathname === '/admin' ? 'bg-yellow-500 text-black' : 'bg-yellow-500/10 text-yellow-500 border border-yellow-500/20'}`}
                    >
