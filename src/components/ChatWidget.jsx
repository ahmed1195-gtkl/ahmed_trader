...
const ChatWidget = () => {
  ...
  return (
    <>
      ...
      <motion.button
        onClick={toggleChat}
        className="fixed bottom-6 right-6 z-50 w-16 h-16 bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-full shadow-lg shadow-yellow-500/30 flex items-center justify-center hover:scale-110 transition-transform duration-300"
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="bg-black/95 backdrop-blur-xl border border-yellow-500/20 rounded-[2rem] shadow-2xl shadow-yellow-500/10 overflow-hidden">
              {/* رأس النافذة */}
              ...
              {!isMinimized && (
                <>
                  ...
                  {/* حقل الإدخال */}
                  <form onSubmit={handleSend} className="border-t border-yellow-500/20 p-4">
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-zinc-900/50 border border-zinc-800 rounded-full px-4 py-2 text-white placeholder-zinc-500 focus:outline-none focus:border-yellow-500/50 transition-colors"
                        disabled={loading}
                      />
                      <button
                        type="submit"
                        disabled={loading || !newMessage.trim()}
                        className="w-10 h-10 bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-full flex items-center justify-center hover:scale-110 transition-transform disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                      >
                        {loading ? (
